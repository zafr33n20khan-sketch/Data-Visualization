# -*- coding: utf-8 -*-
"""Data Visualization Assesment.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1oyEjyZD7ySnRW294C9HWQtvEiTJe38uF
...
df_raw['Confectionary'] = df_raw['Confectionary'].str.strip()

# Fix spelling + standardisation
df_raw['Confectionary'] = df_raw['Confectionary'].replace({
...
# 1. Create synthetic data based on your chart
...
for city in cities:
    for metric in metrics:
        # Generate random data points to create the box distributions
        # Using lognormal distribution to simulate skewed financial data more realistically
        points = np.random.lognormal(mean=np.log(2000), sigma=0.8, size=50) # Adjusted for better data spread

        # Add specific outliers seen in your chart (e.g., very high revenue transactions)
        if city == 'London' and metric == 'Revenue(£)':
            points = np.append(points, [21000, 22500, 23000, 25000]) # Added more pronounced outliers
        elif city == 'Paris' and metric == 'Profit(£)':
            points = np.append(points, [13000, 14000])

        temp_df = pd.DataFrame({'City': city, 'Amount (£)': points, 'Metric': metric})
        data.append(temp_df)

df = pd.concat(data)

# 2. Build the Interactive Box Plot
fig = px.box(
    df,
    x="City",
    y="Amount (£)",
    color="Metric",
    points="outliers",  # Show only outlier points (matching your image)
    title="<b>City-Level Financial Distribution with Outlier Analysis</b><br><span style='font-size:14px;color:gray;'>Distribution of Revenue, Cost, and Profit across European Cities</span>",
    color_discrete_map={
        'Revenue(£)': '#4C78A8', # A professional blue
        'Cost(£)': '#E15759',    # A professional red
        'Profit(£)': '#59A14F'   # A professional green
    },
    height=600
)

# 3. Enhance Layout for professionalism
fig.update_layout(
    template="plotly_white",
    boxmode='group', # Group boxes for the same city together
    yaxis_title="<b>Amount (£)</b>", # Bold Y-axis title
    legend_title="<b>Metric</b>",    # Bold legend title
    hovermode="x unified", # Unified hover for better comparison across metrics in one city
    margin=dict(l=50, r=50, t=100, b=50), # Adjust margins for better spacing
    font=dict(family="Arial", size=12, color="#333333") # Consistent font styling
)
color_discrete_sequence=px.colors.qualitative.Set2
# Further Y-axis formatting
fig.update_yaxes(
    tickprefix="£", # Add currency symbol
    gridcolor='#E0E0E0', # Lighter grid lines
    griddash='dot' # Dotted grid lines
)

# Further X-axis formatting
fig.update_xaxes(
    tickangle=0,
    showline=True,
    linewidth=1,
    linecolor='#cccccc' # Lighter line color
)
...
#Comparison of Profit Trends Across European Cities (3-Month Moving Average)

df_raw['Date'] = pd.to_datetime(df_raw['Date'])

# Aggregate monthly profit for each city
city_monthly = (
    df_raw
...
# Calculate 3-month moving average within each city
...
for ax in g.axes.flatten():
    if ax is not None and ax.has_data():
        ...
        # Y-axis scientific notation (10^x style)
...
# Shared axis labels
g.fig.text(0.5, 0.04, "Year", ha="center", fontsize=15)
g.fig.text(
    0.02, 0.5,
    "Profit (£) – 3-Month Moving Average",
    va="center",
    rotation="vertical",
    fontsize=15
)
color_discrete_sequence=px.colors.qualitative.Set2
# Save and show
plt.savefig("Smoothed_Monthly_Profit_Trends_by_City_Scientific.jpg", dpi=300, bbox_inches="tight")
plt.show()
...
#Profit trends over time by City

# Step 1: Aggregate totals
total_revenue = df_raw['Revenue(£)'].sum()
total_cost = df_raw['Cost(£)'].sum()
total_profit = df_raw['Profit(£)'].sum()

# Step 2: Convert to percentage of revenue
cost_pct = (total_cost / total_revenue) * 100
profit_pct = (total_profit / total_revenue) * 100

# Step 3: Labels and values
labels = ['Revenue (100%)', 'Cost', 'Profit']
values = [100, -cost_pct, profit_pct]
...
# Step 7: Titles
plt.title('Revenue to Profit Breakdown (Normalized %)', fontsize=16)
plt.ylabel('Percentage (%)', fontsize=12)
plt.xlabel('Financial Components', fontsize=12)
color_discrete_sequence=px.colors.qualitative.Set2
plt.grid(axis='y', linestyle='--', alpha=0.6)
plt.tight_layout()

plt.savefig("Waterfall_Normalized_Percentage.jpg", dpi=300)
plt.show()
...
sns.boxplot(x='Confectionary', y='Profit(£)', data=df_raw)
plt.title('Profit Distribution by Product Type')
plt.xticks(rotation=45)
color_discrete_sequence=px.colors.qualitative.Set2
plt.grid(True, linestyle='--', alpha=0.8)
plt.savefig("Profit_by_Confectionary_1.jpg", dpi=800)
plt.show()

#Profit by Confectionary
plt.figure(figsize=(12,6))
sns.violinplot(x='Confectionary', y='Profit(£)', data=df_raw)
plt.title('Profit Density by Product Type')
plt.xticks(rotation=45)
plt.grid(True, linestyle='--', alpha=0.8)
color_discrete_sequence=px.colors.qualitative.Set2
plt.savefig("Profit_by_Confectionary_2.jpg", dpi=800)
plt.show()
...
sns.heatmap(
    ...
    annot_kws={"size": 10},
    mask=pivot < threshold   # 🔥 highlights strong performers
)
...
plt.legend(handles[:len(df_raw['Confectionary'].unique())+1],
           labels[:len(df_raw['Confectionary'].unique())+1],)
color_discrete_sequence=px.colors.qualitative.Set2
...
#Revenue vs Cost
sns.scatterplot(x='Revenue(£)', y='Cost(£)', data=df_raw)
plt.title('Revenue vs Cost')
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()
color_discrete_sequence=px.colors.qualitative.Set2
plt.savefig("Revenue_vs_Cost.jpg", dpi=1000)
plt.show()
...
fig = px.treemap(
    df_raw,
    path=['City', 'Confectionary'], # Define the hierarchy: City then Confectionary
    values='Revenue(£)', # The size of the rectangles will be based on Revenue(£)
    title='Revenue Portfolio Mix by City and Confectionary Type',
...
# Calculate the percentage share of each Confectionary type
confectionary_share = df_raw['Confectionary'].value_counts()

# Create the pie chart
plt.figure(figsize=(8, 8))
confectionary_share.plot.pie(autopct='%1.1f%%', startangle=90, cmap='viridis')
plt.title('Percentage Share of Confectionery Types')
plt.ylabel('') # Hide the default 'Confectionary' label on the y-axis
plt.tight_layout()
color_discrete_sequence=px.colors.qualitative.Set2
plt.savefig("Confectionery_Share_Pie_Chart.jpg", dpi=300)
plt.show()
...
# Create df_city by aggregating data from df_raw and adding coordinates
...
df_city = geo # Assign the prepared geo DataFrame to df_city

# Square-root scaling makes bubble areas easier to compare
# Clamp negatives to zero so bubble area is always meaningful
df_city['Bubble_Size'] = np.sqrt(df_city['Profit'].clip(lower=0))
...
# scatter_mapbox does NOT allow marker.line, so only opacity is used
fig.update_traces(
    marker=dict(
        opacity=0.78
    ),
    ...
    customdata=np.stack([df_city['Profit'], df_city['Revenue'], df_city['Units_Sold'], df_city['Profit_Margin']], axis=-1)
)
color_discrete_sequence=px.colors.qualitative.Set2
fig.update_layout(
    ...
    title=dict(
        text="European Profit Distribution by City<br>" +
             "<span style='font-size:13px;color:gray'>" +
             "Bubble size = Profit | Colour = Profit Margin</span>",
        x=0.5,
        xanchor='center'
    ),
...
df_city['Profit_Margin'] = (df_city['Profit'] / df_city['Revenue'] * 100).round(1)
df_city['lat'] = df_city['City'].map(lambda c: CITY_COORDS.get(c, (np.nan, np.nan))[0])
df_city['lon'] = df_city['City'].map(lambda c: CITY_COORDS.get(c, (np.nan, np.nan))[1])
df_city.dropna(subset=['lat', 'lon'], inplace=True)

# ── Square-root scaling for proportional bubble areas ─────────────────────────
df_city['Bubble_Size'] = np.sqrt(df_city['Profit'].clip(lower=0))
...
fig.update_traces(
    ...
    customdata=np.stack(
        [df_city['Profit'], df_city['Revenue'],
         df_city['Units_Sold'], df_city['Profit_Margin']], axis=-1
    )
)
...
fig.update_layout(
    ...
    coloraxis_colorbar=dict(title="Profit Margin (%)", ticksuffix="%"),
    font=dict(family="Arial")
)
...
if not geo.empty:
    fig_map = px.scatter_mapbox(
        geo, lat='lat', lon='lon',
        # Pass raw profit ─ px will scale bubble AREA proportionally to size_max
        size='Profit_pos',
        size_max=60,
        color='Margin',
        hover_name='City',
...
product_profit = df_raw.groupby('Confectionary')['Profit(£)'].sum().sort_values(ascending=False).reset_index()
...
fig.add_trace(
    go.Pie(
        labels=product_profit['Confectionary'],
...
insight_text = (
    ...
    f"   {top_product['Confectionary']}<br>"
...
product_revenue = df_raw.groupby('Confectionary')['Revenue(£)'].sum().sort_values(ascending=False).reset_index()
        ...
        args=[
            # Update all three pie traces
            {
                'labels': [
                    ...
                    product_profit['Confectionary'].tolist(),
                    city_profit['City'].tolist()
                ],
                'values': [
        ...
        args=[
            {
                'labels': [
                    ...
                    product_revenue['Confectionary'].tolist(),
                    city_revenue['City'].tolist()
                ],
                'values': [
...
for _, row in product_profit.iterrows():
    print(f"   {row['Confectionary']}: £{row['Profit(£)']:,.0f} ({row['Percentage']}%)")
...
product_dropdown = widgets.Dropdown(
    options=df_raw['Confectionary'].unique(),
    description='Product:'
)
...
# ============================================================================
# FULLY IMMERSIVE EUROPEAN INTERACTIVE DASHBOARD
# ============================================================================
...
# Fill Profit / Revenue / Cost where two of three exist
...
def build(city, product, metric, year):
    ...
    with output:
        ...
        p_min, p_max = geo['Profit'].min(), geo['Profit'].max()
        if p_max > p_min:
            geo['BubbleSize'] = (
                ((geo['Profit'] - p_min) / (p_max - p_min)) ** 0.4 * 90 + 10
            )
        else:
            geo['BubbleSize'] = 50
        ...
        display(HTML(fig_donut.to_html(include_plotlyjs='cdn', full_html=False)))
